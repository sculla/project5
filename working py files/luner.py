...
class AGENT:
    ...
    def TRAIN(self):
        ...
        checkpoint = ModelCheckpoint(filepath, monitor='self.myloss', verbose=1, save_best_only=True, mode='max',
                                     period=1)
...
if __name__ == '__main__':
    ...
    while load_version < 150:
        save_version = 1 + load_version
        l_link = "output/weights/LunarLander/1/{}LunarLander-v2.h5".format(load_version)
        s_link = "output/weights/LunarLander/1/{}LunarLander-v2.h5".format(save_version)
        ...
        agent.s_link = s_link
        agent.l_link = l_link
        print(agent.l_link, agent.s_link)
        agent.e_ += .01

        error = []
        seed = random.seed(666)
